			...
			accounts.map(async (account) => {
				try {
					...
					orderHistory.forEach((order) => {
						...
						const profitPercentage = parseFloat(((profit / accountSize) * 100).toFixed(4));
...
<body>
    <div class="email-container">
		...
		if (emailSent) {
			await ConsistencyBreakModel.updateMany({ account: account }, { $set: { isDisabled: true } });
		}
...
const sendConsistencyBreakWarningEmail = async (account, accountDetails) => {
	try {
		const tickets = accountDetails.trades
			.map(
				(trade) => `Ticket: ${trade.ticket}, Profit: ${trade.profit} (${trade.profitPercentage}%)`
			)
			.join(", ");
		const htmlContent = `<!DOCTYPE html>
            <html lang="en">
		...
		if (typeof info === "string" && info.includes("OK")) {
			...
			await ConsistencyBreakModel.updateMany({ account: account }, { $set: { emailSent: true } });
		}
