...
const { StoreData } = require("../breach/breach.schema");
...
const assignNewMT5Account = async (
	...
	try {
		...
		let updatedLeverage;
		if (newChallengeStage === "funded") {
			updatedLeverage = 50;
		} else if (newChallengeStage === "phase2") {
			updatedLeverage = 100;
		}

		const order = await MOrder.findOne({
			orderId: purchasedProduct.productId,
		});
		...
		await StoreData.findOneAndUpdate(
			{},
			{
				$push: {
					dailyData: {
