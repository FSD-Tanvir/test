...
const {
  invoiceMailingHTMLTemplate,
} = require("../../helper/utils/invoiceMailingHTMLTemplate");
  ...
  try {
    ...
    if (paymentStatus === "Paid") {
      ...
      if (
        ...
        updatedOrder?.discountPrice ===
        updatedOrder?.orderItems[0].challengePrice
      ) {
        const invoiceHTML = await invoiceMailingHTMLTemplate(updatedOrder);
        await sendEmailSingleRecipient(
          updatedOrder?.buyerDetails?.email,
    ...
    const matchingAccount =
      user &&
      (await user.mt5Accounts.find((account) => account.productId === orderId));
...
const totalOrderSales = async () => {
  try {
    ...
    const [totalSalesResult] = await MOrder.aggregate(
      totalSalesPipeline
    ).exec();
    const [todaySalesResult] = await MOrder.aggregate(
      todaySalesPipeline
    ).exec();
  ...
  try {
    ...
    for (const order of orders) {
      ...
      if (orderCreationTime >= twoHoursAgo && orderCreationTime < oneHourAgo) {
        ...
        console.log(
          "Follow-up email sent successfully to:",
          buyerDetails.email
        );
      }
    }
