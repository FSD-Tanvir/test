...
} = require("../challengePass/challengePass.services.js");
const {
...
} = require("../breach/breach.schema.js");
...
const handleMatchTraderAccountCreate = async (userDetails) => {
  ...
  try {
    const createMatchTraderAccount = await createTradingAccountAndDeposit(
      userDetails
    );
...
const generateAllUsersCSV = async () => {
  try {
    ...
    const users = await MUser.find(
      {},
      "email first last mt5Accounts createdAt"
    ).lean();
  ...
  try {
    ...
    const filteredData = users.flatMap((user) => {
      const filteredAccounts = user.mt5Accounts.filter((account) => {
        const stageMatch =
          !challengeStage || account.challengeStage === challengeStage;
...
const getAllUsers = async (
  ...
  try {
    ...
    const isVerifiedFilter =
      isVerified === "true" ? true : isVerified === "false" ? false : null;
    ...
    const pipeline = [
      ...
      ...(isVerifiedFilter !== null
        ? [{ $match: { isVerified: isVerifiedFilter } }]
        : []),
...
const getPhasedUsers = async (account) => {
  try {
    ...
    const mt5Account = user.mt5Accounts.find(
      (acc) => acc.account === Number(account)
    );
      ...
      const newMt5Accounts = userData.mt5Accounts.filter(
        (newAcc) =>
          !user.mt5Accounts.some(
            (existingAcc) => existingAcc.account === newAcc.account
          )
      );
...
const updatePurchasedProducts = async (userId, productData) => {
  try {
    ...
    if (!productData.productId || !productData.product) {
      throw new Error(
        "Both `productId` and `product` fields are required in the product data."
      );
    }
...
const manualChallengePass = async (id) => {
  try {
    ...
    const { challengeStage, challengeStageData, userId, account } =
      singleAccount;
    ...
    if (matchingAccount) {
      ...
      const changeGroup = await accountUpdate(
      ...
      if (changeGroup === "OK") {
        ...
        if (disableMT5Account === "OK") {
          ...
          await handleNextChallengeStage(
            matchingAccount,
            user,
            matchingAccount
          );
    ...
    return {
      ...
      status: 500,
      message:
        "An internal error occurred while processing the manual challenge pass.",
      error: error,
    };
  }
};
