...
const getCheckout = async (req, res) => {
	...
	try {
		...
		return res
			.status(200)
			.json({ message: "Checkout fetched successfully", data: result });
	} catch (error) {
		console.error("Error fetching checkout:", error);
		return res
			.status(500)
			.json({ message: "Error fetching checkout", error: error.message });
	}
};
...
const checkMt5Account = async (req, res) => {
	...
	try {
		...
		return res
			.status(200)
			.json({ message: "Account data fetched successfully", data: result });
	} catch (error) {
...
module.exports = { createCheckout, getCheckout, checkMt5Account };
